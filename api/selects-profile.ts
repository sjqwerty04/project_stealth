...
import { MAX_LABEL_CHARS, meaningfulStats, normalizeStats, parseSelectsProfile, templateProfile, type TasteStats } from '../src/lib/onboarding/profile.js';
...
function parseRequest(body: unknown): ProfileRequest | null {
  ...
  const list = (v: unknown) =>
    Array.isArray(v) ? v.filter((x): x is string => typeof x === 'string').slice(0, 5).map((x) => x.slice(0, MAX_LABEL_CHARS)) : [];
  return {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!meaningfulStats(request.stats)) return res.status(200).json({ profile: fallback, source: 'template', reason: 'empty' });
  try {
