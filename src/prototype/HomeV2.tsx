...
export default function HomeV2() {
  ...
  if (eventsLoading) {
    return (
      <div className="h-screen w-full flex items-center justify-center bg-black text-gray-500">
  ...
  return (
    <div className="h-[100dvh] bg-[#09090b] font-sans text-gray-100 flex flex-col max-w-md mx-auto shadow-2xl overflow-hidden border-x border-gray-800 relative">

      {/* ─── Header ─── */}
      <div className="bg-[#09090b]/90 backdrop-blur-md px-5 py-4 flex items-center justify-between z-10 border-b border-gray-800 flex-shrink-0">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-white">Selects</h1>
          <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">Your Cinema Journal</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => navigate('/discover')} className="p-2 hover:bg-gray-800 rounded-full transition-colors text-gray-400 hover:text-white">
            <Search size={18} />
          </button>
        ...
        {recLoading && !recommendation ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6">
            ...
            <div className="text-center">
              <p className="text-sm font-medium text-white mb-1">Finding your next watch...</p>
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6">
            ...
            <button onClick={() => generateRecommendation()} className="text-xs px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors">
              Try Again
            </button>
          </div>
        ) : recommendation ? (
          <>
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-6 pb-2 gap-4 min-h-0">
              ...
              >
                <img
                  ...
                  className="w-44 h-auto max-h-[50vh] object-cover rounded-2xl border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.6)] group-hover:scale-[1.02] transition-transform duration-300"
                />
              ...
              <div className="text-center space-y-1.5 max-w-[80%]">
                <h2 className="text-xl font-bold text-white leading-tight">{recommendation.title}</h2>
                ...
                {recommendation.genres && recommendation.genres.length > 0 && (
                  <div className="flex gap-1.5 justify-center flex-wrap">
                    {recommendation.genres.map((g) => (
                      <span key={g} className="text-[10px] px-2 py-0.5 rounded-full bg-white/5 text-gray-400 border border-white/10">
                        {g}
                      </span>
                    ))}
                  </div>
            ...
            {/* Action Buttons */}
            <div className="flex border-t border-gray-800/60 bg-black/30 backdrop-blur-sm flex-shrink-0">
              <button
                onClick={() => handleRate('up')}
                disabled={isRating || isSkipping}
                className={`flex-1 py-3 flex items-center justify-center gap-1.5 text-xs font-medium transition-all border-r border-gray-800/60 ${
                  showSuccess === 'up' ? 'bg-green-500 text-white' : 'text-green-400 hover:bg-green-900/30'
                } disabled:opacity-50`}
              >
              ...
              <button
                ...
                disabled={isRating || isSkipping}
                className={`flex-1 py-3 flex items-center justify-center gap-1.5 text-xs font-medium transition-all border-r border-gray-800/60 ${
                  showSuccess === 'down' ? 'bg-red-500 text-white' : 'text-red-400 hover:bg-red-900/30'
                } disabled:opacity-50`}
              >
              ...
              <button
                onClick={handleSkip}
                disabled={isRating || isSkipping}
                className="flex-1 py-3 flex items-center justify-center gap-1.5 text-xs font-medium text-gray-400 hover:bg-gray-800/50 transition-all border-r border-gray-800/60 disabled:opacity-50"
              >
            ...
            <button
              ...
              disabled={recLoading}
              className="absolute top-4 right-5 p-2 text-gray-500 hover:text-white hover:bg-gray-800/60 rounded-full transition-colors disabled:opacity-50 z-10"
            >
      ...
      {/* ─── Date Strip ─── */}
      <div className="flex-shrink-0 bg-[#0c0c0e] border-t border-gray-800/60 pb-[env(safe-area-inset-bottom)]">
        {/* Month/Year label */}
          ...
          {dateRange.map((date, i) => {
            ...
            return (
              <div
                ...
                className={`flex-shrink-0 w-14 h-16 rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all relative ${
                  isSelected
                    ? hasEvent
                    ...
                    : hasEvent
                      ...
                      : isToday
                        ...
                        : 'bg-[#18181b] hover:bg-gray-800 border border-transparent'
                }`}
              ...
              >
                ...
                {dayEvts.length > 1 && (
                  <span className="absolute -top-1 -right-1 text-[8px] font-bold w-4 h-4 rounded-full bg-white/25 backdrop-blur-sm flex items-center justify-center" style={{ color: accentText }}>
                    {dayEvts.length}
                  </span>
                )}
                {hasEvent && dayEvts.length === 1 && (
                  <div className="absolute -bottom-0.5 w-1.5 h-1.5 rounded-full bg-white/40" />
                )}
              </div>
            );
          })}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          <div className="bg-[#18181b] w-full max-w-md rounded-t-3xl sm:rounded-2xl p-6 shadow-2xl z-50 relative animate-in slide-in-from-bottom-full duration-300 border border-gray-800 overflow-hidden max-h-[85vh] overflow-y-auto">
                  ...
                  {dayEvents.map((event) => (
                    ...
                    >
                      ...
                      <div className="flex flex-col justify-center flex-1">
                        ...
                        <span className="text-xs text-gray-500">{event.year} {event.runtimeLabel && `\u2022 ${event.runtimeLabel}`}</span>
                      </div>
                      {event.rating && (
                        <div className={`self-center px-2 py-1 rounded-full text-xs font-bold ${event.rating === 'up' ? 'bg-green-900/50 text-green-400' : 'bg-red-900/50 text-red-400'}`}>
                          {event.rating === 'up' ? '\uD83D\uDC4D' : '\uD83D\uDC4E'}
                        </div>
                      )}
                    </div>
                ...
                <div className="max-h-80 overflow-y-auto space-y-2 scrollbar-thin scrollbar-thumb-gray-700">
                  {hasQuery ? (
                    isSearchingMovies ? (
                      ...
                      <div className="text-sm text-gray-500 p-4">No matches for &ldquo;{searchQuery}&rdquo;. Try another title.</div>
                    ) : (
                      filteredMovies.map((movie) => (
                        ...
                        >
                          ...
                          <div className="flex flex-col justify-center">
                            ...
                            <span className="text-xs text-gray-500">{movie.year} \u2022 {movie.runtime}</span>
                          </div>
                        </div>
                      ))
                    )
            ...
            {viewMode === 'confirm' && selectedMovie && (
              <div className="space-y-6">
                <div className="relative rounded-2xl overflow-hidden border border-gray-800 bg-gray-900">
                  ...
                  <div className="relative p-4 flex gap-4 items-center">
                    ...
                    <div className="flex-1">
                      ...
                      <button onClick={() => { setViewMode('search'); setSearchQuery(''); }} className="inline-flex items-center gap-1 text-xs text-blue-300 font-medium mt-3 hover:text-white transition-colors">
                        Change Movie
                      </button>
                    </div>
                  </div>
                ...
                {isPast ? (
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button onClick={() => setRating('up')} className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${rating === 'up' ? 'border-green-500 bg-green-900/20 text-green-400' : 'border-gray-800 bg-gray-900 text-gray-500 hover:bg-gray-800'}`}>
                        <ThumbsUp size={32} className={rating === 'up' ? 'fill-current' : ''} />
                        <span className="mt-2 font-bold text-sm">Loved it</span>
                      </button>
                      <button onClick={() => setRating('down')} className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${rating === 'down' ? 'border-red-500 bg-red-900/20 text-red-400' : 'border-gray-800 bg-gray-900 text-gray-500 hover:bg-gray-800'}`}>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-3">
                      <div className={`flex items-center justify-between p-4 rounded-xl border-2 cursor-pointer transition-all ${!inviteFriend ? 'border-blue-600 bg-blue-900/10' : 'border-gray-800 bg-gray-900'}`} onClick={() => setInviteFriend(false)}>
                        <div className="flex items-center gap-3">
                          <div className="bg-gray-800 p-2 rounded-full text-gray-300"><UserIcon size={18} /></div>
                          <span className="font-medium text-gray-200">Just Me</span>
                        </div>
                        {!inviteFriend && <Check size={20} className="text-blue-500" />}
                      </div>
                      <div className={`p-4 rounded-xl border-2 transition-all ${inviteFriend ? 'border-purple-600 bg-purple-900/10' : 'border-gray-800 bg-gray-900'}`}>
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex items-center gap-3 cursor-pointer" onClick={() => setInviteFriend(true)}>
                            <div className="bg-gray-800 p-2 rounded-full text-purple-400"><Users size={18} /></div>
                          ...
                          <button type="button" onClick={() => setInviteFriend(true)} className="text-xs font-semibold px-3 py-1 rounded-full border border-purple-500 text-purple-300 hover:bg-purple-500/20 transition-colors">
                            Invite
                          </button>
                        </div>
                      </div>
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-xl font-bold text-lg shadow-lg transform transition-transform active:scale-95 flex items-center justify-center gap-2 ${(isPast && !rating) || isSaving ? 'bg-gray-800 text-gray-500 cursor-not-allowed' : 'bg-white hover:bg-gray-200 text-black'}`}
                >
            ...
            {viewMode === 'details' && selectedMovie && (
              <div className="space-y-6">
                <div className="absolute top-4 right-4 z-20 flex gap-3">
                  <button onClick={() => navigate(`/movie/${selectedMovie.id}?type=${selectedMovie.mediaType || 'movie'}`)} className="w-10 h-10 bg-blue-500 text-white rounded-full flex items-center justify-center shadow-lg hover:bg-blue-600 hover:scale-105 transition-all" title="View Full Details">
                    <Info size={18} />
                  </button>
                  <button onClick={() => setViewMode('search')} className="w-10 h-10 bg-amber-400 text-black rounded-full flex items-center justify-center shadow-lg hover:bg-amber-300 hover:scale-105 transition-all border border-amber-500/20" title="Swap Movie">
                    <Repeat size={18} strokeWidth={2.5} />
                  </button>
                  <button onClick={handleDeleteEvent} className="w-10 h-10 bg-red-500 text-white rounded-full flex items-center justify-center shadow-lg hover:bg-red-600 hover:scale-105 transition-all" title="Delete Event">
                    <Trash2 size={18} />
                  </button>
                </div>
                  ...
                  <div className="absolute bottom-0 left-0 p-4 flex gap-3 items-end w-full">
                    ...
                    <div className="mb-1 flex-1">
                      {movieLogoUrl ? (
                        ...
                        <h3 className="font-bold text-white text-xl leading-none mb-1">{selectedMovie.title}</h3>
                      )}
                      <p className="text-gray-300 text-xs">{selectedMovie.runtime}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {viewMode === 'success' && lastSavedMovie && (
              <div className="space-y-6 text-center">
                <div className="flex flex-col items-center py-4">
                  ...
                  <p className="text-gray-400 text-sm">{selectedDate?.toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}</p>
                </div>
                <div className="flex gap-4 items-center bg-gray-900 p-4 rounded-xl border border-gray-800">
                  ...
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center ${lastSavedRating === 'up' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
