  ...
  return (
   <div className="min-h-screen bg-base text-fg flex flex-col items-center justify-center p-6 max-w-md mx-auto">
    ...
    <button
     ...
     className="px-4 py-2 bg-gray-800 hover:bg-gray-700 text-fg text-sm min-h-11 min-w-11"
            aria-label="Go back"
    >
     Back to Library
    </button>
  ...
  <div className="min-h-screen bg-base font-display text-fg flex flex-col max-w-md mx-auto shadow-2xl overflow-hidden border-x border-line">
   ...
   <div className="bg-base/90 backdrop-blur-md px-4 py-4 flex items-center justify-between sticky top-0 z-10 border-b border-line">
    <div className="flex items-center gap-3 min-w-0">
     <button
      onClick={() => navigate('/watchlist')}
      className="p-2 text-fg-2 hover:text-fg hover:bg-gray-800 transition-colors shrink-0"
     >
      <ArrowLeft size={20} />
     </button>
     <div className="min-w-0">
      <h1 className="text-lg font-bold text-fg truncate">{list?.name ?? 'Loading…'}</h1>
      <button
       onClick={() => setShowMembers(true)}
       className="text-xs text-fg-3 hover:text-fg-2 flex items-center gap-1.5 mt-0.5"
      >
       <Users size={12} />
    ...
    <div className="flex items-center gap-2 shrink-0">
     ...
     <div className="flex -space-x-2">
      ...
      {extraCount > 0 && (
       <div className="w-7 h-7 ring-2 ring-black/70 bg-gray-800 text-xs flex items-center justify-center font-semibold">
        +{extraCount}
       </div>
      )}
     </div>
     <button
      ...
      className="ml-1 p-2 bg-blue-600 hover:bg-blue-500 text-fg transition-colors"
      aria-label="Invite"
     >
      <Share2 size={16} />
     </button>
   ...
   <div className="flex-1 overflow-y-auto p-4">
    {loading ? (
     ...
     <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h3 className="text-lg font-medium text-fg-2 mb-2">No movies yet</h3>
      ...
      <div className="flex flex-col gap-3 w-full max-w-xs">
       <button
        ...
        className="flex items-center justify-center gap-2 px-4 py-3 bg-white text-black font-medium transition-colors hover:bg-gray-200"
       >
        <Search size={18} />
        Add a movie
       </button>
       <button
        onClick={() => setShowInvite(true)}
        className="flex items-center justify-center gap-2 px-4 py-3 bg-gray-900 border border-line text-fg-2 font-medium transition-colors hover:bg-gray-800"
       >
        <Plus size={18} />
        Invite a friend
       </button>
     ...
     <>
      ...
      <button
       onClick={() => navigate('/discover')}
       className="w-full mb-4 flex items-center justify-center gap-2 py-2.5 border border-dashed border-line text-fg-3 hover:text-fg-2 hover:border-gray-600 transition-colors text-sm"
      >
       <Search size={15} />
     ...
     <div className="bg-base-2 w-full max-w-sm p-6 shadow-2xl z-50 relative border border-line">
      <div className="flex gap-4 mb-4">
       ...
       <div className="flex-1">
        ...
        <div className="flex items-center gap-2 mt-2 text-xs text-fg-3">
         <AddedByChip
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
     ...
     <div className="bg-base-2 w-full max-w-sm p-6 shadow-2xl z-50 relative border border-line">
      <div className="flex items-center justify-between mb-4">
       <h3 className="font-bold text-fg text-lg">Members</h3>
       <button onClick={() => setShowMembers(false)} className="p-2 text-fg-2 hover:text-fg hover:bg-gray-800 ">
        <X size={16} />
       </button>
      </div>
      ...
      <div className="space-y-2 mb-4 max-h-72 overflow-y-auto">
       {Object.entries(list.members ?? {}).map(([uid, m]) => (
        <div key={uid} className="flex items-center gap-3 bg-black/30 p-3 border border-line">
         <AddedByChip displayName={m.displayName} handle={m.handle} photoURL={m.profileImage} size={36} />
         <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-fg truncate">{m.displayName}</p>
          {m.handle && <p className="text-xs text-fg-3">@{m.handle}</p>}
         </div>
         {uid === list.ownerUid ? (
          <span className="text-[10px] uppercase tracking-wider text-blue-400 bg-blue-500/10 border border-blue-500/30 px-2 py-0.5">
           Owner
          </span>
         ) : isOwner ? (
          <button
