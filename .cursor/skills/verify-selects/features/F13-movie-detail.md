...
- One shared document per film at `film_axes/{mediaType}:{id}` holds `schema`, the film identity, the axes, and `createdAt`. Read and create require auth. Update and delete are denied, so the first reader's reading is the film's reading.
- The rule checks the document it is handed: exactly those seven fields, `schema` 1, a `movie` or `tv` string, an integer `filmId`, a path key that is `mediaType + ':' + filmId`, and a list of eight axes. A document cannot be filed under a film it does not name.
